...
// Global variables
std::unordered_map<std::size_t, std::size_t> dof_component_map_u, dof_component_map_l;
...
void extract_dof_component_map_user(const FunctionSpace& Z, std::string flag)
{
  ...
  if (flag == "F") {
    if (element.value_rank() == 0)
      extract_dof_component_map(dof_component_map_u, Z, &component);
    else
      {printf("Poisson problem: only scalar elements should be there"); exit(-1);}
  }

  if (flag == "S") {
    if (element.value_rank() == 0)
      extract_dof_component_map(dof_component_map_l, Z, &component);
    else
      {printf("Poisson problem: only scalar elements should be there"); exit(-1);}
  }
}
...
void interpolate_delta(const Function& u0, Function& u, std::string abc, std::string meshid) 
{
  ...
  for (std::size_t p = 0; p < num_processes; ++p)
  {
      ...
      for (const auto &d : dofs)
      {
          ...
          {   
            if (element.value_rank() == 0)
              {local_u_vector[d] += vals[j*u0.value_size() + dof_component_map_l[d]];}
            else
              {printf("Poisson problem: only scalar elements should be there"); exit(-1);}
          }
          else if (meshid == "F")
          {
            if (element.value_rank() == 0)
              {local_u_vector[d] += vals[j*u0.value_size() + dof_component_map_u[d]];}
            else
              {printf("Poisson problem: only scalar elements should be there"); exit(-1);}
          }
      }            
