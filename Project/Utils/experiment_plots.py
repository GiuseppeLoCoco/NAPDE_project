...
def print_spatial_convergence_table(
    ...
    if errs_L2_p is not None:
        ...
        print("=" * 105)
        print(f"{'n':>5} | {'h':>8} | {'L2(u) Err':>14} | {'Rate':>6} | {'H1(u) Err':>14} | {'Rate':>6} | {'L2(p) Err':>14} | {'Rate':>6}")
        print("-" * 105)
        for i, n in enumerate(resolutions):
            ...
            print(f"{n:5d} | {h_vals[i]:8.4f} | {errs_L2_u[i]:14.5e} | {r_l2_str} | {errs_H1_u[i]:14.5e} | {r_h1_str} | {errs_L2_p[i]:14.5e} | {r_p_str}")
        print("=" * 105 + "\n")
    else:
        print("\n" + "=" * 85)
        print(f"CONVERGENCE SUMMARY TABLE: {method_name.upper()} BUFFER RECOVERY")
        print("=" * 85)
        print(f"{'n':>5} | {'h':>8} | {'L2(u) Err':>14} | {'Rate':>6} | {'H1(u) Err':>14} | {'Rate':>6}")
        print("-" * 85)
        for i, n in enumerate(resolutions):
            r_l2_str = f"{rates_L2[i-1]:+6.2f}" if i > 0 else "    --"
            r_h1_str = f"{rates_H1[i-1]:+6.2f}" if i > 0 else "    --"
            print(f"{n:5d} | {h_vals[i]:8.4f} | {errs_L2_u[i]:14.5e} | {r_l2_str} | {errs_H1_u[i]:14.5e} | {r_h1_str}")
        print("=" * 85 + "\n")
...
def print_strategy_b_table(
    ...
    if errs_L2_p is not None:
        ...
        print("=" * 105)
        print(f"{'n':>5} | {'h':>8} | {'Scaled R':>12} | {'L2(u) Err':>14} | {'Rate':>6} | {'H1(u) Err':>14} | {'Rate':>6} | {'L2(p) Err':>14} | {'Rate':>6}")
        print("-" * 105)
        for i, n in enumerate(resolutions):
            r_l2_str = f"{rates_L2[i-1]:+6.2f}" if i > 0 else "    --"
            r_h1_str = f"{rates_H1[i-1]:+6.2f}" if i > 0 else "    --"
            r_p_str = f"{rates_p[i-1]:+6.2f}" if (i > 0 and len(rates_p) >= i) else "    --"
            print(f"{n:5d} | {h_vals[i]:8.4f} | {scaled_R_vals[i]:12.2e} | {errs_L2_u[i]:14.5e} | {r_l2_str} | {errs_H1_u[i]:14.5e} | {r_h1_str} | {errs_L2_p[i]:14.5e} | {r_p_str}")
        print("=" * 105 + "\n")
    else:
        print("\n" + "=" * 85)
        print("CONVERGENCE SUMMARY TABLE: SPATIAL CONVERGENCE WITH SCALED BRINKMAN PENALTY R(h)")
        print("=" * 85)
        print(f"{'n':>5} | {'h':>8} | {'Scaled R':>12} | {'L2(u) Err':>14} | {'Rate':>6} | {'H1(u) Err':>14} | {'Rate':>6}")
        print("-" * 85)
        for i, n in enumerate(resolutions):
            r_l2_str = f"{rates_L2[i-1]:+6.2f}" if i > 0 else "    --"
            r_h1_str = f"{rates_H1[i-1]:+6.2f}" if i > 0 else "    --"
            print(f"{n:5d} | {h_vals[i]:8.4f} | {scaled_R_vals[i]:12.2e} | {errs_L2_u[i]:14.5e} | {r_l2_str} | {errs_H1_u[i]:14.5e} | {r_h1_str}")
        print("=" * 85 + "\n")
...
def plot_phase_comparison_loglog(
    ...
    """
    Plots normalized log-log spatial convergence comparing Phase 1 (Conforming) and Phase 2 (Buffer recovery).
    ...
    # Subplot 1: Normalized L2 Velocity (P2 -> O(h^3))
    norm_p1_l2 = np.array(res_p1["L2_u"]) / res_p1["L2_u"][0]
    norm_p2_l2 = np.array(res_p2["L2_u"]) / res_p2["L2_u"][0]
    ax1.loglog(h_arr, norm_p1_l2, 'o-', color='#1f77b4', linewidth=2, label=r'Phase 1: Conforming $L^2(\mathbf{u})$')
    ax1.loglog(h_arr, norm_p2_l2, 's--', color='#d62728', linewidth=2, label=f'Phase 2: {method_name} $L^2(\\mathbf{{u}})$')
    ax1.loglog(h_arr, (h_arr / h0)**3, 'k-', alpha=0.7, label=r'Optimal $O(h^3)$ ($P_2$)')
    ax1.loglog(h_arr, (h_arr / h0)**2, 'k:', alpha=0.6, label=r'Slope $O(h^2)$')
    ax1.set_xlabel(r"Mesh Size $h = L_x/n$", fontsize=11)
    ax1.set_ylabel(r"Normalized Error $E(h)/E(h_0)$", fontsize=11)
    ax1.set_title(r"Velocity $L^2$ Convergence (Slope $O(h^3)$)", fontsize=12, fontweight='bold')
    ax1.grid(True, which="both", linestyle="--", alpha=0.5)
    ax1.legend(fontsize=9)

    # Subplot 2: Normalized H1 Velocity (P2 -> O(h^2))
    norm_p1_h1 = np.array(res_p1["H1_u"]) / res_p1["H1_u"][0]
    norm_p2_h1 = np.array(res_p2["H1_u"]) / res_p2["H1_u"][0]
    ax2.loglog(h_arr, norm_p1_h1, 'o-', color='#1f77b4', linewidth=2, label=r'Phase 1: Conforming $H^1(\mathbf{u})$')
    ax2.loglog(h_arr, norm_p2_h1, 's--', color='#d62728', linewidth=2, label=f'Phase 2: {method_name} $H^1(\\mathbf{{u}})$')
    ax2.loglog(h_arr, (h_arr / h0)**2, 'k-', alpha=0.7, label=r'Optimal $O(h^2)$ ($P_2$)')
    ax2.loglog(h_arr, (h_arr / h0)**1, 'k--', alpha=0.6, label=r'Slope $O(h)$')
    ax2.set_xlabel(r"Mesh Size $h = L_x/n$", fontsize=11)
    ax2.set_ylabel(r"Normalized Error $E(h)/E(h_0)$", fontsize=11)
    ax2.set_title(r"Velocity $H^1$ Convergence (Slope $O(h^2)$)", fontsize=12, fontweight='bold')
    ...
    if has_p:
        ...
        ax3.loglog(h_arr, norm_p2_p, 's--', color='#d62728', linewidth=2, label=f'Phase 2: {method_name} $L^2(p)$')
        ax3.loglog(h_arr, (h_arr / h0)**2, 'k-', alpha=0.7, label=r'Optimal $O(h^2)$ ($P_1$)')
        ax3.loglog(h_arr, (h_arr / h0)**1, 'k--', alpha=0.6, label=r'Slope $O(h)$')
        ax3.set_xlabel(r"Mesh Size $h = L_x/n$", fontsize=11)
        ax3.set_ylabel(r"Normalized Error $E(h)/E(h_0)$", fontsize=11)
...
def plot_spatial_convergence_with_interface(
    ...
    """
    Plots normalized spatial convergence for a single buffer method (L2, H1 and interface trace error).
    """
    h_arr = np.array(h_vals)
    h0 = h_arr[0]
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 5.5))
    fig.suptitle(f"Normalized Spatial Convergence: Upstream Buffer Recovery via {method_name}", fontsize=13, fontweight='bold')

    # Subplot 1: Domain errors
    ax1.loglog(h_arr, np.array(errs_L2_u) / errs_L2_u[0], 'o-', color='#1f77b4', linewidth=2, label=r'Velocity $L^2(\Omega_0)$ Error')
    ax1.loglog(h_arr, np.array(errs_H1_u) / errs_H1_u[0], 's--', color='#ff7f0e', linewidth=1.8, label=r'Velocity $H^1(\Omega_0)$ Error')
    ax1.loglog(h_arr, (h_arr / h0)**3, 'k-', alpha=0.7, label=r'Optimal $O(h^3)$ ($P_2$)')
    ax1.loglog(h_arr, (h_arr / h0)**2, 'k:', alpha=0.7, label=r'Optimal $O(h^2)$ ($P_2$)')
    ax1.set_xlabel(r"Mesh Size $h = L_x/n$", fontsize=11)
    ax1.set_ylabel(r"Normalized Error $E(h)/E(h_0)$", fontsize=11)
    ...
    ax2.loglog(h_arr, np.array(errs_intf) / errs_intf[0], 'd-', color='#d62728', linewidth=2, label=r'Interface Trace $L^2(\Sigma)$ Error')
    ax2.loglog(h_arr, (h_arr / h0)**2, 'k:', alpha=0.6, label=r'Reference $O(h^2)$')
    ax2.loglog(h_arr, (h_arr / h0)**1, 'k--', alpha=0.6, label=r'Reference $O(h)$')
    ax2.set_xlabel(r"Mesh Size $h = L_x/n$", fontsize=11)
    ax2.set_ylabel(r"Normalized Trace Error $E(h)/E(h_0)$", fontsize=11)
...
def plot_strategy_b_spatial_convergence(
    ...
    h0 = h_arr[0]
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 5.5))
    fig.suptitle(r"Strategy B: Normalized Spatial Convergence ($R(h) \sim h^{-2}$)", fontsize=13, fontweight='bold')
