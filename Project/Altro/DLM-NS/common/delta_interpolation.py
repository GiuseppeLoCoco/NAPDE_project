...
std::unordered_map<std::size_t, std::size_t> dof_component_map_f1, dof_component_map_f2, dof_component_map_s1, dof_component_map_s2;
    // map from global dof to the component of the vector field it encodes
    // *1 = vectorial (values = 0, 1, ..., dim), *2 = scalar (value = 0 for all dofs)
...
tabulate_coordinates_to_values_and_nodal_volumes(const Function& u0)
{
  ...
  for (VertexIterator vert(mesh0); !vert.end(); ++vert)    
    ...
    u0.eval(_node_val, _node_coord);      // Remember this is a known bug here.  ->  could be this https://answers.launchpad.net/fenics/+question/207055
...
void interpolate_delta(const Function& u0, Function& u, std::string abc, std::string meshid) 
{
  ...
  std::map<std::vector<double>, std::pair<std::vector<double>, double>> 
      coords_to_values_and_nodal_volumes = tabulate_coordinates_to_values_and_nodal_volumes(u0);    // NODAL VOLUMES (TRIANGLES) = 1/3 * TOTAL AREA OF TRIANGLES SHARING THIS VERTEX
  ...
  for (std::size_t p = 0; p < num_processes; ++p)
  {
      ...
      for (std::size_t j = 0; j < points.size()/gdim0; ++j)     // MESH1: LOOP OVER POINTS
      {
        std::copy(points.begin() + j*gdim0, points.begin() + (j + 1)*gdim0,
                x.begin());
	    
		    // ................ Delta function interpolation ..................
		    m_m = create_mini_bounding_box(x, h, 1e-12);    // BOUNDING BOX +-3*h AROUND x

		    // Initializing values as zero
		    for(std::size_t i = 0; i < gdim0; i++)
		        _values[i] = 0;
		    
		    for (const auto &map : coords_to_values_and_nodal_volumes)    // MESH0: LOOP OVER POINTS (first) WITH VAL (second.first) AND NODAL_VOL (second.second)
			  {
              ...
              nodal_volume = pr.second;
  		       	...
  		       	for(std::size_t i = 0; i < gdim0; i++)
  		         	_values[i] += _node_val[i]*delta(gdim0, h, _x, node_coord, abc)*nodal_volume;     //  ++ u0(x0)*delta(x1)*nodal_volume(x0)
  		      }
		    }
		    // ................. Delta function interpolation ..................
  ...
  for (std::size_t p = 0; p < num_processes; ++p)
  {
      ...
      for (const auto &d : dofs)
      {
          ...
          {
            if (element.value_rank() == 0)
              {local_u_vector[d] += vals[j*u0.value_size() + dof_component_map_f2[d]];}   // HP: VECTOR IS STORED AS (..., ux(xk),uy(xk),uz(xk), ux(xk+1),uy(xk+1),...)
            if (element.value_rank() == 1)
