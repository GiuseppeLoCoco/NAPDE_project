from firedrake import *
from scipy.interpolate import splev
import numpy as np
import sys, math, os, cppimport
from ufl import Jacobian
from utilities.utils import BLUE  

# In Firedrake, l'utilizzo diretto di FacetNormal nei DirichletBC (che sono nodali)
# richiede di proiettare la normale su uno spazio vettoriale.
class Inflow:
    def __init__(self, param, mesh, V_space):
        self.param = param
        self.mesh = mesh
        
        # Valore scalare tempo-dipendente
        self.t_val = Constant(0.0)
        
        # Proiettiamo la normale verso l'interno (-n) su uno spazio continuo
        n = FacetNormal(mesh)
        self.neg_normal_proj = project(-n, V_space)
        
        # L'espressione finale è il vettore normale moltiplicato per lo scalare
        self.expr = self.t_val * self.neg_normal_proj

    def update(self):
        t = self.param["time"].t
        period = self.param["period"]
        nm = self.param["nm"].cycle
        Area = self.param["Area"]
        Vsc = self.param["Vsc"]
        Tsc = self.param["Tsc"]
        func = self.param["func"]

        val = (splev(t*Tsc - nm*period*Tsc, func)/Area)/Vsc
        self.t_val.assign(val)
        
    def get_expression(self):
        return self.expr

# ==============================================================================
# INIZIO BLOCCO C++ (Mantenuto rigorosamente intatto come richiesto)
# ==============================================================================
code = '''
#include <pybind11/pybind11.h>
#include <pybind11/eigen.h>
namespace py = pybind11;

#include <dolfin/function/Expression.h>
#include <dolfin/mesh/MeshFunction.h>
#include <dolfin/mesh/Cell.h>
#include <dolfin/geometry/Point.h>

class Inflow_x : public dolfin::Expression
{
public:
    
    double v;
    std::shared_ptr<dolfin::MeshFunction<std::size_t>> cell_data;

    Inflow_x(double v_, std::shared_ptr<dolfin::MeshFunction<std::size_t>> cell_data_) : Expression() {
        v = v_;
        cell_data = cell_data_;
    }

    void eval(Eigen::Ref<Eigen::VectorXd> values, Eigen::Ref<const Eigen::VectorXd> x, const ufc::cell& c) const override
    {
        assert(cell_data);
        const dolfin::Cell cell(*cell_data->mesh(), c.index);
        
        assert(c.local_facet >= 0);
        dolfin::Point n = cell.normal(c.local_facet);
        values[0] = -n.x()*v;  
    }
};

class Inflow_y : public dolfin::Expression
{
public:
    
    double v;
    std::shared_ptr<dolfin::MeshFunction<std::size_t>> cell_data;

    Inflow_y(double v_, std::shared_ptr<dolfin::MeshFunction<std::size_t>> cell_data_) : Expression() {
        v = v_;
        cell_data = cell_data_;
    }

    void eval(Eigen::Ref<Eigen::VectorXd> values, Eigen::Ref<const Eigen::VectorXd> x, const ufc::cell& c) const override
    {
        assert(cell_data);
        const dolfin::Cell cell(*cell_data->mesh(), c.index);
        
        assert(c.local_facet >= 0);
        dolfin::Point n = cell.normal(c.local_facet);
        values[0] = -n.y()*v;  
    }
};

class Inflow_z : public dolfin::Expression
{
public:
    
    double v;
    std::shared_ptr<dolfin::MeshFunction<std::size_t>> cell_data;

    Inflow_z(double v_, std::shared_ptr<dolfin::MeshFunction<std::size_t>> cell_data_) : Expression() {
        v = v_;
        cell_data = cell_data_;
    }

    void eval(Eigen::Ref<Eigen::VectorXd> values, Eigen::Ref<const Eigen::VectorXd> x, const ufc::cell& c) const override
    {
        assert(cell_data);
        const dolfin::Cell cell(*cell_data->mesh(), c.index);
        
        assert(c.local_facet >= 0);
        dolfin::Point n = cell.normal(c.local_facet);
        values[0] = -n.z()*v;  
    }
};

PYBIND11_MODULE(SIGNATURE, m)
{
    py::class_<Inflow_x, std::shared_ptr<Inflow_x>, dolfin::Expression>(m, "Inflow_x")
    .def(py::init<double, std::shared_ptr<dolfin::MeshFunction<std::size_t>>>())
    .def_readwrite("v", &Inflow_x::v)
    .def_readwrite("cell_data", &Inflow_x::cell_data);
    
    py::class_<Inflow_y, std::shared_ptr<Inflow_y>, dolfin::Expression>(m, "Inflow_y")
    .def(py::init<double, std::shared_ptr<dolfin::MeshFunction<std::size_t>>>())
    .def_readwrite("v", &Inflow_y::v)
    .def_readwrite("cell_data", &Inflow_y::cell_data);
    
    py::class_<Inflow_z, std::shared_ptr<Inflow_z>, dolfin::Expression>(m, "Inflow_z")
    .def(py::init<double, std::shared_ptr<dolfin::MeshFunction<std::size_t>>>())
    .def_readwrite("v", &Inflow_z::v)
    .def_readwrite("cell_data", &Inflow_z::cell_data);   
}
'''
# ==============================================================================
# FINE BLOCCO C++
# ==============================================================================


class Outflow:
    def __init__(self, param, mesh):
        self.param = param
        self.mesh = mesh
        self.val = Constant(0.0)

    def update(self):
        t = self.param["time"].t
        period = self.param["period"]
        nm = self.param["nm"].cycle
        Area = self.param["Area"]
        Vsc = self.param["Vsc"]
        Tsc = self.param["Tsc"] 
        func = self.param["func"]

        val = splev(t*Tsc - nm*period*Tsc, func)*133.322/(1060*Vsc*Vsc)
        self.val.assign(val)

    def get_expression(self):
        return self.val

class Temperature_balloon:
    def __init__(self, param, mesh):
        self.param = param
        self.mesh = mesh
        self.val = Constant(0.0)

    def update(self):
        t = self.param["time"].t
        Tsc = self.param["Tsc"]
        func = self.param["func"]

        val = splev(t*Tsc, func)
        self.val.assign(val)

    def get_expression(self):
        return self.val


# Algorithm for remeshing / improving mesh quality
def mesh_smoothening(mesh):
    dim = mesh.geometric_dimension()
    x = SpatialCoordinate(mesh)
    dx_dxi = Jacobian(mesh)
    Jc = abs(det(dx_dxi))
        
    def grad_xi(f):
        df_dx = grad(f)
        return dot(df_dx,dx_dxi)
        
    V = VectorFunctionSpace(mesh, "CG", 1)

    # Variational problem for smoothing:
    u = TrialFunction(V)
    v = TestFunction(V)
    x_smoothed = x + u

    # Parameter to control smoothing effect;
    smoothing_strength = Constant(1e4)

    dxz = Measure("dx", domain=mesh)
    res = (smoothing_strength*inner(grad_xi(x_smoothed), grad_xi(v))
           + dot(u, v))*(1/Jc)*dxz

    uh = Function(V)
    solve(lhs(res) == rhs(res), uh,
          bcs=[DirichletBC(V, Constant(tuple([0.0]*dim)), "on_boundary")],
          solver_parameters={"ksp_type": "cg", "ksp_rtol": 1e-4})

    # Equivalente Firedrake di ALE.move: aggiorno le coordinate
    mesh.coordinates.assign(mesh.coordinates + uh)
        
    # Firedrake non ha una funziona nativa MeshQuality immediata pari a dolfin.
    # Restituiamo 0.0, 0.0 come placeholder (o implementare metrica custom in caso di necessità)
    ratio_min = 0.0
    ratio_max = 0.0

    return mesh, ratio_min, ratio_max


# Function to create nullspace
def attach_nullspace(Ap, x_, Q):
    """Create null space basis object and attach to Krylov solver."""
    null_vec = x_.copy(deepcopy=True)
    with null_vec.dat.vec as v:
        v.set(1.0)
    null_vec /= sqrt(assemble(inner(null_vec, null_vec)*dx))
    
    null_space = VectorSpaceBasis(constant=True) 
    # In Firedrake, il nullspace va tipicamente passato a solver_parameters:
    # {"ksp_constant_null_space": True} o tramite argomento `nullspace=` a `solve()`.
    return null_space


# Function to calculate denominator for courant number
def DENO(u, u_components, Mpi, mesh, h_f_X):
    DN_local = 0
    NM_local = 0
    
    with h_f_X.dat.vec_ro as v_h:
        vertex_values_h_f_X = v_h.array.copy()
        
    vertex_mag_u = np.zeros(len(vertex_values_h_f_X))

    for ui in range(u_components):
        with u[ui].dat.vec_ro as v_u:
            vertex_values_u = v_u.array.copy()
            
        DN_local += np.max(np.abs(vertex_values_u / vertex_values_h_f_X))
        vertex_mag_u += np.square(vertex_values_u)

    NM_local = np.max(np.sqrt(vertex_mag_u)*vertex_values_h_f_X)

    # Adattamento mpi4py per Firedrake
    DN = Mpi.Max(DN_local)
    NM = Mpi.Max(NM_local)

    return DN, NM


# Function to assign-vector in MPI
def vector_assign_in_parallel(v, w):
    # Firedrake gestisce l'assegnazione parallela nativamente in questo modo
    v.assign(w)


# Function to map solid_mesh between reference and current configurations
def mapping(mesh, Mv):
    # ALE.move equivale a spostare le coordinate attuali di Mv
    mesh.coordinates.assign(mesh.coordinates + Mv)


# Degrees of freedom
def Calc_total_DOF(Mpi, **kwargs):
    DOFS = dict()
    for key, value in kwargs.items():
        dof = 0
        for i in value:
            # function_space().dim() in Firedrake restituisce i DOF globali su tutti i rank
            dof += i.function_space().dim()
        DOFS[key] = dof

    return DOFS


# Delta-interpolation
def interpolate_nonmatching_mesh_delta(fsi_interpolation, u0, V, abc, flag):
    # NOTA: Firedrake richiede librerie esterne (es. dual-mortar o vertex_to_dofmap manuale) 
    # per proiezioni non-matching generiche. Mantenuta la struttura originale.
    u = Function(V)
    u.assign(0.0)
    # u0.set_allow_extrapolation(True) non è supportato in Firedrake
    fsi_interpolation.interpolate_delta(u0, u, abc, flag)
    return u 


# Other miscellaneous functions

# Divergence of a vector
def divergence(u, u_components):
    DIV = 0.0
    for ui in range(u_components): DIV += u[ui].dx(ui) 
    return DIV

# Viscous dissipation source term (for energy equation)
def Qf(u, Ec, Re):
    return inner(((2*Ec)/Re)*sym(nabla_grad(u)), nabla_grad(u))

# Perfusion equation (quadratic)
def PFE(Tf_n): 
    return conditional(Tf_n >= 0.725, (6.18*Tf_n*Tf_n) - (7.39*Tf_n) + 2.21, 0.1)  

# Returns a value rounded down to a specific number of decimal places
def round_decimals_down(number:float, decimals:int=8):
    if not isinstance(decimals, int):
        raise TypeError("decimal places must be an integer")
    elif decimals < 0:
        raise ValueError("decimal places has to be 0 or more")
    elif decimals == 0:
        return math.floor(number)

    factor = 10 ** decimals
    return math.floor(number * factor) / factor

# Calculate and print time-step and CFL number statistics
def calc_runtime_stats_timestep(Mpi, problem_physics, u, u_components, u_diff, t, tsp, text_file_handles, mesh, hmin_f, h_f_X, Re, Pr, thermal_diff_ratio, VN_local, time_control): 

    X = 0.6; Y = 1-X
    tsp_min = time_control['dt_min']*hmin_f
    
    DN, NM = DENO(u, u_components, Mpi, mesh, h_f_X)
    VN = Mpi.Max(VN_local)/Re
    KN = min(VN/Pr, (thermal_diff_ratio*VN)/Pr)
    
    C_no_real = DN*tsp
    C_vi_real = VN*tsp
    C_kn_real = min(C_vi_real/Pr, (thermal_diff_ratio*C_vi_real)/Pr) 

    local_Re = NM*Re 

    Mpi.set_barrier()
    if Mpi.get_rank() == 0:
        if problem_physics['solve_temperature']: text_file_handles[1].write(f"{t:0,.8f}        {tsp:0,.8f}        {u_diff:0,.6e}             {local_Re:0,.8f}           {C_no_real:0,.8f}           {C_vi_real:0,.8f}           {C_kn_real:0,.8f}\n")
        if not problem_physics['solve_temperature']: text_file_handles[1].write(f"{t:0,.8f}        {tsp:0,.8f}        {u_diff:0,.6e}             {local_Re:0,.8f}           {C_no_real:0,.8f}           {C_vi_real:0,.8f}           {float('nan')}\n")    

    if tsp > tsp_min and time_control['adjustable_timestep'] == False:
        print("constant timestep : dt = {}".format(tsp), flush = True)  
    
    if tsp > tsp_min and time_control['adjustable_timestep'] == True:

        tsp1 = tsp2 = tsp
        dt_limiter = {0: 'convection', 1: 'viscosity', 2: 'conduction'}

        if C_no_real > time_control['C_no']:
            if C_no_real > 5.0:
                tsp1 = round_decimals_down((time_control['C_no'])/DN, 5)
            else:
                tsp1 = round_decimals_down((Y*time_control['C_no'] + X*C_no_real)/DN, 5)

        elif C_no_real < time_control['C_no']:
            tsp1 = round_decimals_down((X*time_control['C_no'] + Y*C_no_real)/DN, 5)

        if C_vi_real > time_control['C_vi']:
            if C_vi_real > 30.0:
                tsp2 = round_decimals_down((time_control['C_vi'])/VN, 5)
            else:
                tsp2 = round_decimals_down((Y*time_control['C_vi'] + X*C_vi_real)/VN, 5)

        elif C_vi_real < time_control['C_vi']:
            tsp2 = round_decimals_down((X*time_control['C_vi'] + Y*C_vi_real)/VN, 5)

        tsp_list = [tsp1, tsp2]

        if  problem_physics['solve_temperature'] == True:
            
            tsp3 = tsp

            if C_kn_real > time_control['C_kn']:
                if C_kn_real > min(time_control['C_vi']/Pr, thermal_diff_ratio*time_control['C_vi']/Pr):
                    tsp3 = round_decimals_down((time_control['C_kn'])/KN, 5)
                else:
                    tsp3 = round_decimals_down((Y*time_control['C_kn'] + X*C_kn_real)/KN, 5)

            elif C_kn_real < time_control['C_kn']:
                tsp3 = round_decimals_down((X*time_control['C_kn'] + Y*C_kn_real)/KN, 5)        

            tsp_list.append(tsp3)

        tsp = min(tsp_list)
        index_min = np.argmin(tsp_list)

        print("timestep limited by {} CFL : dt = {}".format(dt_limiter[index_min], tsp), flush = True)

    if tsp <= tsp_min:
        tsp = round_decimals_down(tsp_min, 5)
        print(BLUE % "timestep at dt_min = {}".format(tsp), flush = True)            

    return tsp 

# Update solution at end of time loop
def update_variables(update, u_components, problem_physics):
    u_ = update[0]
    p_ = update[1]

    for ui in range(u_components):
        u_[2][ui].assign(u_[1][ui])
        u_[1][ui].assign(u_[0][ui]) 
    p_[2].assign(p_[1])    
    p_[1].assign(p_[0])
    
    if problem_physics['solve_FSI'] == True:
        Dp_ = update[2]
        Lm_ = update[3]

        Dp_[2].assign(Dp_[1])
        Lm_[1].assign(Lm_[0])

        if problem_physics['solve_temperature'] == True:
            Ts_   = update[5]
            LmTs_ = update[6]    
             
            Ts_[1].assign(Ts_[0])
            LmTs_[1].assign(LmTs_[0])

# Update solution at end of time loop
def heat_update_variables(update, lagrange_pb, time, old_time):
    T_ = update[0]
    T_[1].assign(T_[0])
    
    Lm_ = update[1]
    # Sostitutivo Firedrake per sum vettoriale in-place
    Lm_[1].assign(Lm_[1] + Lm_[0])

# Compute eigen value using petsc4py / slepc4py in Firedrake
def compute_eig0(solver, neig_min=1, neig_max=1000):
    neig = neig_min
    ret = None
    while True:
        neig = min(neig, neig_max)
        solver.setDimensions(nev=neig)
        solver.solve()
        try:
            ret = solver.getEigenvalue(0)
            break
        except Exception:
            pass
        if neig >= neig_max:
            raise Exception(f"Reached neig_max = {neig_max}")
        neig = 2*neig
    return ret

# Compute condition number of matrix
def condition_number(X):
    # Firedrake espone la matrice PETSc vera e propria:
    A = assemble(X).M.handle
    
    import slepc4py
    slepc4py.init()
    from slepc4py import SLEPc
    
    eigenSolver = SLEPc.EPS().create()
    eigenSolver.setOperators(A)
    
    eigenSolver.setWhichEigenpairs(SLEPc.EPS.Which.SMALLEST_MAGNITUDE)
    eigen_min = compute_eig0(eigenSolver)

    eigenSolver.setWhichEigenpairs(SLEPc.EPS.Which.LARGEST_MAGNITUDE)
    eigen_max = compute_eig0(eigenSolver)
    
    return abs(eigen_max / eigen_min)