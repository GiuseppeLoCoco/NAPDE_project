...
def get_inflow_profile(mesh, obstacle):
    X = SpatialCoordinate(mesh)
    y = X[1]
    if obstacle.type_obstacle == "cylinder":
    ...
    elif obstacle.type_obstacle == "square":
...
def create_boundary_conditions(fluid_mesh, obstacle, **V):
    inflow_profile = get_inflow_profile(fluid_mesh.mesh, obstacle = obstacle)
...
def create_bcs_penalty(W, mesh, obstacle):
    """
    ...
    """
    inflow_profile = get_inflow_profile(mesh, obstacle = obstacle)
...
def create_bcs_conforming(W, mesh, w_obstacle_velocity, obstacle): # t_constant is now handled by t_param global
    """
