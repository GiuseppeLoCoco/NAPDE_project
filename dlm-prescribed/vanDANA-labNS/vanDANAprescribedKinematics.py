...
def vanDANA_solver(args):
    ...
    Mv = Function(VectorFunctionSpace(solid_mesh.mesh, 'P', fem_degree['displacement_degree'])) # only if keep_solid_on_reference
            ...
            try: 	# loop for recoveries
                ...
                if counters[2] >= print_control['c']:
                    ...
                    if Mpi.get_rank() == 0:
                        ...
                        if t < 0.98*T: text_file_handles[3].write(timings(TimingClear.keep, [TimingType.wall]).str(True))
