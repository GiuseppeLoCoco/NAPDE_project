...
parser.add_argument('--xml_materialfilename',type=str,required=True)
...
parser.add_argument('--rv_endo',type=float,required=True)
parser.add_argument('--rv_epi',type=float,required=True)
parser.add_argument('--b_endo', type=float, required=True)
parser.add_argument('--b_epi', type=float, required=True)
parser.add_argument('--xbase',type=str, required=True)
args = parser.parse_args()

print "************* Entering SetBiVFiber_Ce.py *****************"
...
meshfunc = MeshFunction("size_t", mesh, xmlmaterialfilename)

#plot(meshfunc, interactive=True)

DG = FunctionSpace(mesh, 'DG', 0)
matid = Function(DG)
helper = np.asarray(meshfunc.array(), dtype=np.int32)
dm = DG.dofmap()
for cell in cells(mesh):
    helper[dm.cell_dofs(cell.index())] = meshfunc[cell]
matid.vector()[:] = helper
...
al_endo = al_endo*math.pi/180; b_endo = b_endo*math.pi/180; rv_endo = rv_endo*math.pi/180;
al_epi = al_epi*math.pi/180;  b_epi = b_epi*math.pi/180; rv_epi = rv_epi*math.pi/180;

minz =  np.amin(mesh.coordinates()[:,2])
maxz =  np.amax(mesh.coordinates()[:,2])
minx =  np.amin(mesh.coordinates()[:,0])
maxx =  np.amax(mesh.coordinates()[:,0])
...
Sdof_coordinates = S.dofmap().tabulate_all_coordinates(mesh)
n = V.dim()
d = mesh.geometry().dim()
...
ab, dd4 = lap(mesh, boundaries, par1, 1, 'phi_ab')
...
for x_dof, y_dof, z_dof, scl in zip(x_dofs, y_dofs, z_dofs, scalar_dof):
	...
	if(abs(rv[scl]) < 1e-9 and abs(rv[scl] + lv[scl]) < 1e-9):
        ...
        if(matid(dof_coordinates[x_dof]) < 2):  
        	...
        	al_s[scl] = rv_endo*(1 - ds[scl]) - rv_endo*ds[scl]; b_s[scl] = b_endo*(1 - ds[scl]) - b_endo*ds[scl];
	        al_w[scl] = rv_endo*(1 - epi[scl]) + rv_epi*epi[scl]; b_w[scl] = b_endo*(1 - epi[scl]) + b_epi*epi[scl];
...
#func_of_scalar.vector()[:] = ds
#file15 = File("ds.pvd")
#file15 << func_of_scalar
#
#outfile.close()
