...
physical_parameters = dict(
		...
		nu = 0.001, #0.1,									# Dynamic viscosity (kg/m.s)
...
time_control = dict(
		...
		dt = 0.1,  								# Time-step: constant throughout runtime if adjustable-timestep is "False"
		tmax = 5.0,								# Total runtime
		dt_min = 0.0,#0.02,						# Minimum permissible time step in % of min cell size (recommended - 2%)
		adjustable_timestep = False,			# Calculate variable time-step using CFL numbers : used to accelerate temporal solution

		maxit = 10, 								# HEAT: max DLM/FD iterations
		fd_tol = 1e-8							# HEAT: relative tolerance on increment of g
...
def calc_non_dimensional_numbers(g, rho_f, nu, Spht_f, K_f, rho_s, Sm, Ld, nw, Spht_s, K_s, Lsc, Vsc, T0, Tm, Tsc):
		...
		Re = 1.0/nu #rho_f*(Vsc*Lsc)/nu #100 #rho_f*(Vsc*Lsc)/nu            
		Pr = 1 #(Spht_f*nu)/K_f 
