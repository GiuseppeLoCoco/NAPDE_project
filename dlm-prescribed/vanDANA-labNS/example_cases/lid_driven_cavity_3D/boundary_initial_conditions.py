...
class Point_pressure(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[0], 1.0) and near(x[1], 1.0) and near(x[2], 1.0)
...
def fluid_create_boundary_conditions(fluid_mesh, **V):
	...
	# velocity
	bcu_left_x = DirichletBC(V['fluid'][0], Constant(0), boundaries, 1)
	bcu_right_x = DirichletBC(V['fluid'][0], Constant(0), boundaries, 3)
	bcu_bottom_x = DirichletBC(V['fluid'][0], Constant(0), boundaries, 2)
	bcu_top_x = DirichletBC(V['fluid'][0], Constant(1), boundaries, 4)
	bcu_x = [bcu_left_x, bcu_bottom_x, bcu_right_x, bcu_top_x]
	...
	bcu_y = [bcu_left_y, bcu_bottom_y, bcu_right_y, bcu_top_y]

	bcu_left_z = DirichletBC(V['fluid'][0], Constant(0), boundaries, 1)
	bcu_bottom_z = DirichletBC(V['fluid'][0], Constant(0), boundaries, 2)
	bcu_top_z = DirichletBC(V['fluid'][0], Constant(0), boundaries, 4)
	bcu_right_z = DirichletBC(V['fluid'][0], Constant(0), boundaries, 3)
	bcu_z = [bcu_left_z, bcu_bottom_z, bcu_right_z, bcu_top_z]
