...
class Heat_lagrange_multiplier_problem:
    ...
    def __init__(self, constraint_mesh):
        ...
        # T_data_.vector()[:] = 2.0  # data to assign on the constraint domain
        # # T_data_.interpolate(Expression('x[0]*x[0]*x[0] - x[1]*x[1]*x[1]', degree=5))
        # # T_data_.interpolate(Expression('cos(2*pi*x[0])*cos(2*pi*x[1])', degree=5, pi=PI))
        # T_data_.interpolate(
        #     Expression('0.5*(1+cos(pi/(1-0.5)*(sqrt(x[0]*x[0]+x[1]*x[1])-0.5)))', degree=5, pi=PI)
        # )
        ...
        self.ds = Measure("ds", domain=self.mesh, subdomain_data=constraint_mesh.get_mesh_boundaries())
    ...
    def assemble_lagrange_multiplier(self, Lm_old_, T_, dt):
        ...
        A = assemble(Lm * e * dx)
        b = assemble(1.0/dt*(self.T_data_ - T_)*e*dx + Lm_old_*e*dx)
